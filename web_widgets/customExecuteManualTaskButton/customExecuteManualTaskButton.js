...
})().directive('customExecuteManualTaskButton', function() {
    return {
      controllerAs: 'ctrl',
  ...
  this.action = function action() {
    var id;
     if ($scope.properties.action === 'Submit task') {
      id = getUrlParam('id');
      if (id) {
        doRequest('POST', '../API/bpm/comment/', getUserParam());
        doRequest('PUT', '../API/bpm/manualTask/' + getUrlParam('id'), getUserParam());
      } else {
  ...
  function doRequest(method, url, params) {
    ...
    return $http(req)
      .success(function(data, status) {
        ...
        if ($scope.properties.targetUrlOnSuccess && method == 'PUT') {
          redirectIfNeeded();
        }
      })
