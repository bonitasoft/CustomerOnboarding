 ...
 */
public class CoSignGetEnvImpl extends AbstractCoSignGetEnvImpl {
	...
	protected void executeBusinessLogic() throws ConnectorException{
		...
		logger.info(headerLog + "------------------------ Fetching signed document... ------------------");
		
		byte[] pdfBytes = LibSign.getEnvelopeContent(getEnvelopeId());
		
		//Init value return
        DocumentValue documentValue = new DocumentValue("empty");
		
		documentValue = new DocumentValue(pdfBytes, "application/pdf", getFileTargetName());
		
		logger.info(headerLog + "------------------------ Document stored in Bonita ------------------");
	
		//WARNING : Set the output of the connector execution. If outputs are not set, connector fails
		setEnv(documentValue);
		} catch (Exception e) {
